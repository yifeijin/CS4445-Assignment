...
def centering_X(X):
    ...
    '''
    mu = np.mean(X, axis=0)
    substract = np.outer(np.ones(X.shape[0]), mu)
    Xc = X - substract
...
def compute_P(E,v,k):
    '''
        ...
        Output:
            P: the projection matrix, a numpy float matrix of shape p by k. 
    '''
    P = np.empty((E.shape[0], 1))
    for i in range(k):
        P = np.hstack((P, E[:, v.argsort()[-i-1]].real))
    P = np.delete(P, 0, 1)
...
def PCA(X, k=1):
    ...
    # compute covariance matrix Xc
    cov = compute_C(Xc)

    # compute eigen pairs of L
    eig_vectors, eig_values = compute_eigen_pairs(cov)

    # compute the projection matrix 
    P = compute_P(eig_vectors, eig_values, k)
