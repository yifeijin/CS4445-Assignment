...
def compute_mu_image(X):
    ...
    '''
    mu_image = np.empty(4096)
    for i in range(X.shape[1]):
        mu_image[i] = np.mean(X[:, i])
    mu_image = np.asmatrix(mu_image.reshape(64, 64))
    return mu_image
...
def compute_eigen_faces(X, k=20):
    ...
    for i in range(P.shape[1]):
        P_images.append(P[:, i].reshape(64, 64))
...
def load_dataset():
    ...
    X = np.asmatrix(data.data)
    y = np.asmatrix(data.target).T
    images = data.images
