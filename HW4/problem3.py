...
def compute_distance(Xp, q):
    '''
        ...
        Output:
            d: distances between the query image and all the images in Xp. A numpy vector of shape (400,1), where each
            element i, is the Euclidean distance between i-th image in X and the query image.
    '''
    d = np.empty(Xp.shape[0])
    for i in range(Xp.shape[0]):
        d[i] = np.sum((q - Xp[i]) ** 2) ** .5
